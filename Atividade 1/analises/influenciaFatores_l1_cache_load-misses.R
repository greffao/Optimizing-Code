...
fatores <- list(
  Tecnica = c("Sem_Otimizacao", "Loop_Interchange"),
  Alocacao = c("Estatica", "Dinamica"))
...
#Vetor de resultados
L1_Dcache_Load_Misses = c(529955129,32544218,610086516,32711805)

#adiciona os resultados no planejamento de experimentos)
plan.atualizado = add.response(design = plan.person, response = L1_Dcache_Load_Misses)
...
plan.formula = lm(plan.atualizado$L1_Dcache_Load_Misses~(plan.atualizado$Tecnica*plan.atualizado$Alocacao))
